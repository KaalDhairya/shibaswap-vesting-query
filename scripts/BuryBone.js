...
async function fetchAndStore(blockResult) {
    ...
    const data = await queries.buryBoneUsers(blockResult.blockNumber); //quering BuryBone subgraph for this block
    ...
    let doc = await rewardsCollection.findOneAndUpdate({ block_number: blockResult.blockNumber, contract: "BuryBone" }, obj, { new: true, upsert: true });
...
async function main() {
    ...
    const URL = `https://${config.etherscanUrl}/api?module=account&action=txlist&address=${config.contract.BuryBone}&startblock=0&endblock=99999999&sort=asc`;
    ...
    console.log("BuryBone: Already saved blocks: ", op, " New blocks added :", po);
