...
async function main() {
    ...
    if(!fs.existsSync('./outputs/BuryBoneShib')) {
        fs.mkdirSync('./outputs/BuryBoneShib', { recursive: true})
    }

    fs.writeFileSync(
        `./outputs/BuryBoneShib/amounts-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/BuryBoneShib/blacklisted-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/BuryBoneShib/merkle-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
