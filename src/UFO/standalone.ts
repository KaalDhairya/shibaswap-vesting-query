...
async function main() {
    ...
    if(!fs.existsSync('./outputs/UFO')) {
        fs.mkdirSync('./outputs/UFO', { recursive: true})
    }

    fs.writeFileSync(
        `./outputs/UFO/amounts-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/UFO/blacklisted-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/UFO/merkle-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/UFO/lockInfo-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
