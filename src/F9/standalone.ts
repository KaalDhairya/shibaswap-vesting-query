...
async function main() {
    ...
    if(!fs.existsSync('./outputs/F9')) {
        fs.mkdirSync('./outputs/F9', { recursive: true})
    }

    fs.writeFileSync(
        `./outputs/F9/amounts-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/F9/blacklisted-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/F9/merkle-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/F9/lockInfo-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
