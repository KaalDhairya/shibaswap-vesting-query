...
async function main() {
    ...
    if(!fs.existsSync('./outputs/VEMP')) {
        fs.mkdirSync('./outputs/VEMP', { recursive: true})
    }

    fs.writeFileSync(
        `./outputs/VEMP/amounts-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/VEMP/blacklisted-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/VEMP/merkle-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/VEMP/lockInfo-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
