...
async function main() {
    ...
    if(!fs.existsSync('./outputs/ZIG')) {
        fs.mkdirSync('./outputs/ZIG', { recursive: true})
    }

    fs.writeFileSync(
        `./outputs/ZIG/amounts-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/ZIG/blacklisted-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/ZIG/merkle-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
    ...
    fs.writeFileSync(
        `./outputs/ZIG/lockInfo-${options.startBlock}-${options.endBlock}.json`,//-${options.claimBlock}}`, - will enable when subgraph switches to mainnet
        JSON.stringify(
